...
class NBAApiClient:
    ...
    def _handle_response_error(self, e: Exception, context: str) -> Dict[str, Any]:
        ...
        return {"error": f"API error in {context}: {str(e)}", "status_code": getattr(e, "status_code", None)}
        ...
        try:
            ...
            for player in all_players:
                if player_name_lower == player["last_name"].lower():
                    logger.debug(
                        "Found by last name: %s %s (ID: %s)", player["first_name"], player["last_name"], player["id"]
                    )
                    return player

            # 3) Partial match
            logger.debug("No exact match; trying partial match…")
            matched = [p for p in all_players if player_name_lower in f"{p['first_name']} {p['last_name']}".lower()]

            if matched:
                matched.sort(key=lambda p: len(f"{p['first_name']} {p['last_name']}"))
                best = matched[0]
                logger.debug("Best partial match: %s %s (ID: %s)", best["first_name"], best["last_name"], best["id"])
                return best
    ...
    async def get_player_career_stats(
        ...
        try:
            ...
            df: pd.DataFrame = await asyncio.to_thread(_fetch_player_career_stats, player_name, season)
    ...
    async def get_league_leaders(
        ...
        results = []
        for s in seasons:
            df: pd.DataFrame = await asyncio.to_thread(_fetch_league_leaders, s, stat_category_norm, per_mode_norm)
            if df.empty:
                continue
    ...
    async def get_live_scoreboard(
        ...
        try:
            ...
            payload = await asyncio.to_thread(
                fetch_live_boxsc_odds_playbyplaydelayed_livescores, target_date and str(target_date) or None
            )
    ...
    async def get_league_game_log(
        ...
        try:
            ...
            df: pd.DataFrame = await asyncio.to_thread(
                fetch_league_game_log, season, team_name, season_type, date_from, date_to, direction, sorter
            )
    ...
    async def get_games_by_date(
        ...
        try:
            ...
            sb = await asyncio.to_thread(scoreboardv2.ScoreboardV2, game_date=date_str, league_id=league_id)
            ...
            for _, row in game_header.iterrows():
                ...
                game_data = {
                    ...
                    "home_team": {"id": home_team_id, "full_name": get_team_name(home_team_id)},
                    "visitor_team": {"id": visitor_team_id, "full_name": get_team_name(visitor_team_id)},
